...
--
  ...
  begin
    ...
    m_calling_schema           :=
                               substr( holder.gettypename, 1, dot_index - 1 );
      ...
      select distinct procedure_name
                ...
                where owner || object_name in(
                        select     owner || type_name
                              from all_types
                        start with owner = upper( in_owner )           and
                                   type_name = upper( in_object_name )
                        connect by prior supertype_owner = owner   and
                                   prior supertype_name = type_name ) and
                      procedure_name like in_wildcard                 and
